...
NS_LOG_COMPONENT_DEFINE ("OlsrOceanProtocol");

namespace olsr {

/********** OLSR class **********/

NS_OBJECT_ENSURE_REGISTERED (OceanProtocol);

TypeId
OceanProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::olsr::OceanProtocol")
	...
	.AddTraceSource ("Rx", "Receive OLSR packet.",
                     ...
                     "ns3::olsr::OceanProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("Tx", "Send OLSR packet.",
                     MakeTraceSourceAccessor (&OceanProtocol::m_txPacketTrace),
                     "ns3::olsr::OceanProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("RoutingTableChanged", "The OLSR routing table has changed.",
                     MakeTraceSourceAccessor (&OceanProtocol::m_routingTableChanged),
                     "ns3::olsr::OceanProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
void
OceanProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created olsr::OceanProtocol");
  m_helloTimer.SetFunction (&OceanProtocol::HelloTimerExpire, this);
  m_tcTimer.SetFunction (&OceanProtocol::TcTimerExpire, this);
  m_midTimer.SetFunction (&OceanProtocol::MidTimerExpire, this);
  m_hnaTimer.SetFunction (&OceanProtocol::HnaTimerExpire, this);
  m_queuedMessagesTimer.SetFunction (&OceanProtocol::SendQueuedMessages, this);
...
void
OceanProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
...
void OceanProtocol::DoInitialize ()
{
  ...
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      ...
      if (m_recvSocket == 0)
        {
          ...
          m_recvSocket->SetRecvCallback (MakeCallback (&OceanProtocol::RecvOlsr,  this));
      ...
      socket->SetRecvCallback (MakeCallback (&OceanProtocol::RecvOlsr,  this));
...
void OceanProtocol::SetInterfaceExclusions (std::set<uint32_t> exceptions)
{
...
void
OceanProtocol::RecvOlsr (Ptr<Socket> socket)
{
  ...
  OceanMessageList messages;

  while (sizeLeft)
    {
      OceanMessageHeader messageHeader;
  ...
  for (OceanMessageList::const_iterator messageIter = messages.begin ();
       messageIter != messages.end (); messageIter++)
    {
      const OceanMessageHeader &messageHeader = *messageIter;
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case olsr::OceanMessageHeader::HELLO_MESSAGE:
            ...
            case olsr::OceanMessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OLSR node " << m_mainAddress
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != olsr::OceanMessageHeader::HELLO_MESSAGE)
            {
...
///
int
OceanProtocol::Degree (NeighborTuple const &tuple)
{
  int degree = 0;
...
Ipv4Address
OceanProtocol::GetMainAddress (Ipv4Address iface_addr) const
{
...
void
OceanProtocol::RoutingTableComputation ()
{
...
void
OceanProtocol::ProcessHello (const olsr::OceanMessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const olsr::OceanMessageHeader::Hello &hello = msg.GetHello ();
...
void
OceanProtocol::ProcessTc (const olsr::OceanMessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const olsr::OceanMessageHeader::Tc &tc = msg.GetTc ();
  ...
  for (std::vector<Ipv4Address>::const_iterator i = tc.neighborAddresses.begin ();
       i != tc.neighborAddresses.end (); i++)
    {
      ...
      if (topologyTuple != NULL)
        {
          topologyTuple->expirationTime = now + GetMessageVTime(msg);
        }
      else
        {
          ...
          m_events.Track (Simulator::Schedule (DELAY (topologyTuple.expirationTime),
                                               &OceanProtocol::TopologyTupleTimerExpire,
                                               this,
...
void
OceanProtocol::ProcessMid (const olsr::OceanMessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const olsr::OceanMessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
           ...
           tuple != ifaceAssoc.end (); tuple++)
        {
          if (tuple->ifaceAddr == *i
              && tuple->mainAddr == msg.GetOriginatorAddress ())
            {
              ...
              tuple->time = now + GetMessageVTime(msg);
              updated = true;
            }
        }
      if (!updated)
        {
          ...
          Simulator::Schedule (DELAY (tuple.time),
                               &OceanProtocol::IfaceAssocTupleTimerExpire, this, tuple.ifaceAddr);
        }
    }
...
void
OceanProtocol::ProcessHna (const olsr::OceanMessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const olsr::OceanMessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<olsr::OceanMessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
       it != hna.associations.end (); it++)
    {
      ...
      if (tuple != NULL)
        {
          tuple->expirationTime = now + GetMessageVTime(msg);
        }
...
void
OceanProtocol::ForwardDefault (olsr::OceanMessageHeader olsrMessage,
  ...
  // ...or create a new one
  else
    {
      ...
      Simulator::Schedule (OLSR_DUP_HOLD_TIME,
                           &OceanProtocol::DupTupleTimerExpire, this,
                           newDup.address, newDup.sequenceNumber);
    }
}

void
OceanProtocol::QueueMessage (const olsr::OceanMessageHeader &message, Time delay)
{
...
void
OceanProtocol::SendPacket (Ptr<Packet> packet,
                             const OceanMessageList &containedMessages)
{
...
void
OceanProtocol::SendQueuedMessages ()
{
  ...
  OceanMessageList msglist;

  for (std::vector<olsr::OceanMessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
void
OceanProtocol::SendHello ()
{
  NS_LOG_FUNCTION (this);

  olsr::OceanMessageHeader msg;
  Time now = Simulator::Now ();
  ...
  std::vector<olsr::OceanMessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      olsr::OceanMessageHeader::Hello::LinkMessage linkMessage;
...
void
OceanProtocol::SendMid ()
{
  olsr::OceanMessageHeader msg;
  olsr::OceanMessageHeader::Mid &mid = msg.GetMid ();
...
OceanProtocol::SendHna ()
{
  ...
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  olsr::OceanMessageHeader::Hna &hna = msg.GetHna ();

  std::vector<olsr::OceanMessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      olsr::OceanMessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
void
OceanProtocol::AddHostNetworkAssociation (Ipv4Address networkAddr, Ipv4Mask netmask)
{
...
void
OceanProtocol::RemoveHostNetworkAssociation (Ipv4Address networkAddr, Ipv4Mask netmask)
{
...
void
OceanProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
...
bool
OceanProtocol::UsesNonOlsrOutgoingInterface (const Ipv4RoutingTableEntry &route)
{
...
void
OceanProtocol::LinkSensing (const olsr::OceanMessageHeader &msg,
                              const olsr::OceanMessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  ...
  if (link_tuple == NULL)
    {
      ...
      newLinkTuple.time = now + GetMessageVTime(msg);
  ...
  link_tuple->asymTime = now + GetMessageVTime(msg);
  for (std::vector<olsr::OceanMessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
              ...
              if (lt == OLSR_LOST_LINK)
              ...
              else if (lt == OLSR_SYM_LINK || lt == OLSR_ASYM_LINK)
                {
                  NS_LOG_DEBUG (*link_tuple << ": link is SYM or ASYM => should become SYM now"
                                " (symTime being increased to " << now + GetMessageVTime(msg));
                  link_tuple->symTime = now + GetMessageVTime(msg);
  ...
  if (created)
    {
      ...
      m_events.Track (Simulator::Schedule (DELAY (std::min (link_tuple->time, link_tuple->symTime)),
                                           &OceanProtocol::LinkTupleTimerExpire, this,
                                           link_tuple->neighborIfaceAddr));
    }
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Olsr node " << m_mainAddress
                    << ": LinkSensing END");
...
void
OceanProtocol::PopulateNeighborSet (const olsr::OceanMessageHeader &msg,
                                      const olsr::OceanMessageHeader::Hello &hello)
{
...
void
OceanProtocol::PopulateTwoHopNeighborSet (const olsr::OceanMessageHeader &msg,
                                            const olsr::OceanMessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<olsr::OceanMessageHeader::Hello::LinkMessage> LinkMessageVec;
                  ...
                  if (nb2hop_tuple == NULL)
                    {
                      ...
                      new_nb2hop_tuple.expirationTime = now + GetMessageVTime(msg);
                      ...
                      m_events.Track (Simulator::Schedule (DELAY (new_nb2hop_tuple.expirationTime),
                                                           &OceanProtocol::Nb2hopTupleTimerExpire, this,
                                                           new_nb2hop_tuple.neighborMainAddr,
                                                           new_nb2hop_tuple.twoHopNeighborAddr));
                    }
                  else
                    {
                      nb2hop_tuple->expirationTime = now + GetMessageVTime(msg);
                    }
                }
...
void
OceanProtocol::PopulateMprSelectorSet (const olsr::OceanMessageHeader &msg,
                                         const olsr::OceanMessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
                  ...
                  if (existing_mprsel_tuple == NULL)
                    {
                      ...
                      mprsel_tuple.expirationTime = now + GetMessageVTime(msg);
                      AddMprSelectorTuple (mprsel_tuple);

                      // Schedules mpr selector tuple deletion
                      m_events.Track (Simulator::Schedule
                                        (DELAY (mprsel_tuple.expirationTime),
                                        &OceanProtocol::MprSelTupleTimerExpire, this,
                                        mprsel_tuple.mainAddr));
                    }
                  else
                    {
                      existing_mprsel_tuple->expirationTime = now + GetMessageVTime(msg);
                    }
                }
            }
        }
...
void
OceanProtocol::NeighborLoss (const LinkTuple &tuple)
{
  NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                << "s: OLSR Node " << m_mainAddress
...
void
OceanProtocol::AddDuplicateTuple (const DuplicateTuple &tuple)
{
...
void
OceanProtocol::RemoveDuplicateTuple (const DuplicateTuple &tuple)
{
...
void
OceanProtocol::LinkTupleAdded (const LinkTuple &tuple, uint8_t willingness)
{
...
void
OceanProtocol::RemoveLinkTuple (const LinkTuple &tuple)
{
  NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                << "s: OLSR Node " << m_mainAddress
...
void
OceanProtocol::LinkTupleUpdated (const LinkTuple &tuple, uint8_t willingness)
{
...
void
OceanProtocol::AddNeighborTuple (const NeighborTuple &tuple)
{
...
void
OceanProtocol::RemoveNeighborTuple (const NeighborTuple &tuple)
{
...
void
OceanProtocol::AddTwoHopNeighborTuple (const TwoHopNeighborTuple &tuple)
{
...
void
OceanProtocol::RemoveTwoHopNeighborTuple (const TwoHopNeighborTuple &tuple)
{
...
void
OceanProtocol::IncrementAnsn ()
{
  m_ansn = (m_ansn + 1) % (OLSR_MAX_SEQ_NUM + 1);
}

void
OceanProtocol::AddMprSelectorTuple (const MprSelectorTuple  &tuple)
{
...
void
OceanProtocol::RemoveMprSelectorTuple (const MprSelectorTuple &tuple)
{
...
void
OceanProtocol::AddTopologyTuple (const TopologyTuple &tuple)
{
...
void
OceanProtocol::RemoveTopologyTuple (const TopologyTuple &tuple)
{
...
void
OceanProtocol::AddIfaceAssocTuple (const IfaceAssocTuple &tuple)
{
...
void
OceanProtocol::RemoveIfaceAssocTuple (const IfaceAssocTuple &tuple)
{
...
void
OceanProtocol::AddAssociationTuple (const AssociationTuple &tuple)
{
  m_state.InsertAssociationTuple (tuple);
}

void
OceanProtocol::RemoveAssociationTuple (const AssociationTuple &tuple)
{
  m_state.EraseAssociationTuple (tuple);
}

uint16_t OceanProtocol::GetPacketSequenceNumber ()
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (OLSR_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t OceanProtocol::GetMessageSequenceNumber ()
{
...
void
OceanProtocol::HelloTimerExpire ()
{
  SendHello ();
...
void
OceanProtocol::DupTupleTimerExpire (Ipv4Address address, uint16_t sequenceNumber)
{
...
void
OceanProtocol::LinkTupleTimerExpire (Ipv4Address neighborIfaceAddr)
{
  Time now = Simulator::Now ();
...
void
OceanProtocol::Nb2hopTupleTimerExpire (Ipv4Address neighborMainAddr, Ipv4Address twoHopNeighborAddr)
{
  ...
  if (tuple->expirationTime < Simulator::Now ())
    ...
    {
      m_events.Track (Simulator::Schedule (DELAY (tuple->expirationTime),
                                           &OceanProtocol::Nb2hopTupleTimerExpire,
                                           this, neighborMainAddr, twoHopNeighborAddr));
    }
}

void
OceanProtocol::MprSelTupleTimerExpire (Ipv4Address mainAddr)
{
  ...
  if (tuple->expirationTime < Simulator::Now ())
    ...
    {
      m_events.Track (Simulator::Schedule (DELAY (tuple->expirationTime),
                                           &OceanProtocol::MprSelTupleTimerExpire,
                                           this, mainAddr));
    }
}

void
OceanProtocol::TopologyTupleTimerExpire (Ipv4Address destAddr, Ipv4Address lastAddr)
{
...
void
OceanProtocol::IfaceAssocTupleTimerExpire (Ipv4Address ifaceAddr)
{
  ...
  if (tuple->time < Simulator::Now ())
    ...
    {
      m_events.Track (Simulator::Schedule (DELAY (tuple->time),
                                           &OceanProtocol::IfaceAssocTupleTimerExpire,
                                           this, ifaceAddr));
    }
}

void
OceanProtocol::AssociationTupleTimerExpire (Ipv4Address gatewayAddr, Ipv4Address networkAddr, Ipv4Mask netmask)
{
  ...
  if (tuple->expirationTime < Simulator::Now ())
    ...
    {
      m_events.Track (Simulator::Schedule (DELAY (tuple->expirationTime),
                                           &OceanProtocol::AssociationTupleTimerExpire,
...
void
OceanProtocol::RemoveEntry (Ipv4Address const &dest)
{
  m_table.erase (dest);
}

bool
OceanProtocol::Lookup (Ipv4Address const &dest,
...
bool
OceanProtocol::FindSendEntry (RoutingTableEntry const &entry,
                                RoutingTableEntry &outEntry) const
{
  outEntry = entry;
...
Ptr<Ipv4Route>
OceanProtocol::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
...
bool OceanProtocol::RouteInput  (Ptr<const Packet> p,
...
void
OceanProtocol::NotifyInterfaceUp (uint32_t i)
{
}
void
OceanProtocol::NotifyInterfaceDown (uint32_t i)
{
}
void
OceanProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
}
void
OceanProtocol::NotifyRemoveAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
}


void
OceanProtocol::AddEntry (Ipv4Address const &dest,
...
OceanProtocol::GetRoutingTableEntries () const
{
...
int64_t
OceanProtocol::AssignStreams (int64_t stream)
{
...
bool
OceanProtocol::IsMyOwnAddress (const Ipv4Address & a) const
{
...
OceanProtocol::GetRoutingTableAssociation () const
{
